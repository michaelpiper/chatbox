fychatBox = function(c) {
    ...
    // Selectors
    window.user = {};
    window.user.AppUserID = 1000000000;
    ...
    (function() {
        ...
        xhr1.onreadystatechange = function() {
            if (xhr1.readyState === 4) {
                var chatID = JSON.parse(xhr1.responseText);
                if (chatID.success) {
                    window.user = chatID.data;
                } else {
                    window.user.AppUserID = 1000000000;
                }
            }
        }
        xhr1.open('POST', c.api, true)
        xhr1.setRequestHeader('Content-type', "application/x-www-form-urlencoded");
        xhr1.send('action=user');
        /**
         * load chat style
         */
        var xhr = createXHR();
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4) {
                var chatStyleData = JSON.parse(xhr.responseText);
                if (chatStyleData.success) {
                    document.head.insertAdjacentHTML('beforeend', chatStyleData.data);
                } else {

                }
            }
        }
        xhr.open('POST', c.api, true)
        xhr.setRequestHeader('Content-type', "application/x-www-form-urlencoded");
        xhr.send('action=chatStyle');
        ...
        xhr2.onreadystatechange = function() {
            if (xhr2.readyState === 4) {
                var chatTempData = JSON.parse(xhr2.responseText);
                // console.log(chatTempData);
                if (chatTempData.success) {
                    document.body.insertAdjacentHTML('beforeend', chatTempData.data);
                    assignElementval();
                    loadEmoji();
                } else {
        ...
        xhr2.open('POST', c.api, true)
        xhr2.setRequestHeader('Content-type', "application/x-www-form-urlencoded");
        xhr2.send('action=chatTemp');
        /**
         * load chat list
         */
        var xhr3 = createXHR();
        xhr3.onreadystatechange = function() {
            if (xhr3.readyState === 4) {
                var chatListData = JSON.parse(xhr3.responseText);
                if (chatListData.success) {
                    window.ChatMessage = chatListData.data;
                } else {
                    window.ChatMessage = [];
                }
                loadMessage();
        ...
        xhr3.open('POST', c.api, true)
        xhr3.setRequestHeader('Content-type', "application/x-www-form-urlencoded");
        xhr3.send('action=list');
    })()

    function assignElementval() {
        window.messages = document.querySelector('.messages')
        window.btn = document.querySelector('.btn-chat-sender')
        window.input = document.querySelector('.input-chat-sender input')
        window.emojiholder = document.querySelector('.emoji-holder')
        window.emojiwrapper = document.querySelector('.emoji-wrapper')
        window.emojibtn = document.querySelector('.emoji-btn')
            // Button/Enter Key
    ...
    function sendMessage() {
        ...
        if (hours > 12) {
            row.time = hours + ":" + date.getMinutes() + "pm"
        } else {
            row.time = hours + ":" + date.getMinutes() + "am"
        }
    ...
    function addMessage(msg) {
        ...
        xhr4.onreadystatechange = function() {
            if (xhr4.readyState === 4) {
                var chatSendData = JSON.parse(xhr4.responseText);
                if (chatSendData.success) {
        ...
        xhr4.open('POST', c.api, true)
        xhr4.setRequestHeader('Content-type', "application/x-www-form-urlencoded");
        xhr4.send('action=send&msg=' + msg);
    }
    ...
    function writeLine(text, attr) {
        var message = document.createElement('div')
        message.classList.add('message');
        var chat = document.createElement('div')
        var detail = document.createElement('div')
        if (attr.type == 'receiver') {
            chat.classList.add('chat-receiver');
        } else {
            chat.classList.add('chat-sender')
        }
        if (attr.type == "receiver")
            text = attr.sender + ": " + text;
        chat.innerHTML = text;
        detail.classList.add('chat-detail');
        detail.innerHTML = attr.time;
...
`
    ...
    loadMessage = function() {
        clearMsg();
        if (user.AppUserID == 1000000000 && ChatMessage.length < 1) {
            var row = {};
            var date = new Date();
            var hours = date.getHours()
            if (hours > 12) {
            ...
            ChatMessage.push({
                sender: "Robot",
                receiver: "you",
                msg: "you are chatting as anonymous your message will not be save to your account",
                senderID: 90000900009,
                time: row.time,
                date: row.date
            })
        }
        for (i in ChatMessage) {
            attr = {
                time: ChatMessage[i].time,
                date: ChatMessage[i].date,
                receiver: ChatMessage[i].receiver,
                sender: ChatMessage[i].sender
            }

            if (ChatMessage[i].senderID == user.AppUserID) {
                ChatMessage[i].msg = ChatMessage[i].msg;
                attr.type = '';
                writeLine(ChatMessage[i].msg, attr);
            } else {
                attr.type = 'receiver';
                writeLine(ChatMessage[i].msg, attr);
            }
        ...
        xhr3.onreadystatechange = function() {
            if (xhr3.readyState === 4) {
                ...
                if (chatListData.success) {
                    ...
                    window.ChatMessage = ChatMessage ? ChatMessage : [];
                }
                loadMessage();
            }
        }
